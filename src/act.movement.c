...
int do_simple_move(struct char_data *ch, int dir, int need_specials_check)
{
  ...
  if (!IS_AFFECTED(ch, AFF_SNEAK)) {
    if (AFF2_FLAGGED(ch, AFF2_HASTE))
	    sprintf(buf2, "$n leaves %s quickly.", dirs[dir]);
    else
...
int perform_move(struct char_data *ch, int dir, int need_specials_check)
{
  ...
  if (ch == NULL || dir < 0 || dir >= NUM_OF_DIRS)
  ...
  else if (IS_SET(EXIT(ch, dir)->exit_info, EX_CLOSED) && !AFF3_FLAGGED(ch, AFF3_PASSDOOR)) {
	...
	if (perform_move(k->follower, dir, 1)) 
...
ACMD(do_enter)
{
  ...
  if (*buf) {			/* an argument was supplied, search for door
	...
	if (EXIT(ch, door)->to_room != NOWHERE)
	  if ((!IS_SET(EXIT(ch, door)->exit_info, EX_CLOSED) || AFF3_FLAGGED(ch, AFF3_PASSDOOR))&&
