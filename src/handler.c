...
void affect_total(struct char_data * ch)
{
  ...
  if (IS_NPC(ch) || IS_MONK(ch) || IS_WARRIOR(ch) || IS_BARBARIAN(ch) || IS_SAMURAI(ch)) {
...
void obj_to_char(struct obj_data * object, struct char_data * ch)
{
  if (object && ch) {
    ...
    if (IS_OBJ_STAT2(object, ITEM2_AUTOENGRAVE) && !IS_OBJ_STAT2(object, ITEM2_ENGRAVED) && !IS_NPC(ch)) {
	strcpy(object->owner_name, GET_NAME(ch));
	SET_BIT(GET_OBJ_EXTRA2(object), ITEM2_ENGRAVED);
    }
...
void equip_char(struct char_data * ch, struct obj_data * obj, int pos)
{
  ...
  if (invalid_class(ch, obj)) {
	act("Your class is prevented from using this item.", FALSE, ch, obj, 0, TO_CHAR);
	obj_to_char(obj, ch);
	return;
  }
  if (invalid_race(ch, obj)) {
	act("Your race is prevented from using this item.", FALSE, ch, obj, 0, TO_CHAR);
	obj_to_char(obj, ch);
	return;
  }
...
*/
  (obj_index[GET_OBJ_RNUM(obj)].number[obj->orig_zone]) -= 1;
...
/*	!str_cmp(i->player.name, name) && CAN_SEE(ch, i)) */
	myisname(i->player.name, name) && CAN_SEE(ch, i))
      return i;
...
struct char_data *get_char_vis(struct char_data * ch, char *name)
{
  ...
  for (i = character_list; i && (j <= number); i = i->next)
    if (myisname(tmp, i->player.name) && CAN_SEE(ch, i) && 
