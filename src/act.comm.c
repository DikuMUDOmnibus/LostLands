...
ACMD(do_say)
{
  ...
  if (!*argument)
    ...
    MOBTrigger = FALSE;
    if (!IS_MOB(ch) || MOB3_FLAGGED(ch, MOB3_CANTALK))
...
ACMD(do_tell)
{
  ...
  if (!*buf || !*buf2)
    ...
    act("$E is away from $S keyboard right now.  Please try again later.", FALSE, ch, 0, vict, TO_CHAR | TO_SLEEP);
  else
...
ACMD(do_gen_comm)
{
  ...
  static char *com_msgs[][4] = {
    ...
    {"You cannot gossip!!\r\n",
      "gossip",
      "You aren't even on the gossip channel!\r\n",
    KYEL},
    ...
    {"You cannot do racial chatter!!\r\n",
      "chat",
      "You aren't even on your race-specific chat channel!\r\n",
    KMAG},

    {"You cannot encode on the war channel.\r\n",
      "encode",
      "You aren't even on the war channel.\r\n",
    KBLU},
  ...
  if (subcmd == SCMD_ENCODE && !(PRF2_FLAGGED(ch, PRF2_WAR_DRUHARI) ||
		PRF2_FLAGGED(ch, PRF2_WAR_YLLANTRA))) {
	send_to_char("You're not registered for the war.\r\n", ch);
	return;
  }
  if (subcmd == SCMD_BROADCAST && !(PRF2_FLAGGED(ch, PRF2_ARENA_RED) ||
		PRF2_FLAGGED(ch, PRF2_ARENA_BLUE))) {
  ...
  for (i = descriptor_list; i; i = i->next) {
	...
	continue;
      if (subcmd == SCMD_CHAT && (GET_RACE(ch) != GET_RACE(i->character)) &&
		!IS_IMMORT(i->character))
        continue;
      if (subcmd == SCMD_ENCODE && !( (PRF2_FLAGGED(ch, PRF2_WAR_DRUHARI) &&
		PRF2_FLAGGED(i->character, PRF2_WAR_DRUHARI)) ||
		(PRF2_FLAGGED(i->character, PRF2_WAR_YLLANTRA) &&
		PRF2_FLAGGED(ch, PRF2_WAR_YLLANTRA)) ))
	continue;
      if (subcmd == SCMD_BROADCAST && !( (PRF2_FLAGGED(ch, PRF2_ARENA_RED) &&
		PRF2_FLAGGED(i->character, PRF2_ARENA_RED)) ||
		(PRF2_FLAGGED(i->character, PRF2_ARENA_BLUE) &&
		PRF2_FLAGGED(ch, PRF2_ARENA_BLUE)) ))
	continue;
