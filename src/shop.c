...
int is_ok_char(struct char_data * keeper, struct char_data * ch, int shop_nr)
{
  ...
  if ((IS_MAGIC_USER(ch) && NOTRADE_MAGIC_USER(shop_nr)) ||
      (IS_WIZARD(ch) && NOTRADE_MAGIC_USER(shop_nr)) ||
      (IS_CLERIC(ch) && NOTRADE_CLERIC(shop_nr)) ||
      (IS_DRUID(ch) && NOTRADE_CLERIC(shop_nr)) ||
      (IS_THIEF(ch) && NOTRADE_THIEF(shop_nr)) ||
      (IS_NINJA(ch) && NOTRADE_THIEF(shop_nr)) ||
      (IS_BARBARIAN(ch) && NOTRADE_WARRIOR(shop_nr)) ||
      (IS_SAMURAI(ch) && NOTRADE_WARRIOR(shop_nr)) ||
...
int trade_with(struct obj_data * item, int shop_nr)
{
  ...
  if (IS_OBJ_STAT(item, ITEM_NOSELL) || IS_OBJ_STAT2(item, ITEM2_ENGRAVED))
    return (OBJECT_NOTOK);
		       ...
		       struct char_data * keeper, int shop_nr)
{
  ...
  while ((obj) && ((GET_GOLD(ch) >= buy_price(obj, shop_nr)) || IS_GOD(ch))
    ...
    if (shop_producing(obj, shop_nr))
      obj = read_object(GET_OBJ_RNUM(obj), REAL, world[ch->in_room].zone);
    else {
      obj_from_char(obj);
      SHOP_SORT(shop_nr)--;
    }
		            ...
		            struct char_data * keeper, int shop_nr, int msg)
{
  ...
  if (!(obj = get_obj_in_list_vis(ch, name, ch->carrying))) {
	 ...
	 if (invalid_class(ch, obj)) {
		sprintf(buf, "%s Sorry, your class is not allowed to use that.",
			GET_NAME(ch));
		do_tell(keeper, buf, cmd_tell, 0);
	 } else if (invalid_race(ch, obj)) {
		sprintf(buf, "%s Sorry, your race is not allowed to use that.",
			GET_NAME(ch));
		do_tell(keeper, buf, cmd_tell, 0);
	 } else {
		         ...
		         struct char_data * keeper, int shop_nr)
{
  ...
  one_argument(arg, name);
  if ((obj = get_selling_obj(ch, name, keeper, shop_nr, TRUE))) {
	  sprintf(buf, "%s I'll give you %d coins for that!", GET_NAME(ch),
		  sell_price(ch, obj, shop_nr));
	  do_tell(keeper, buf, cmd_tell, 0);
  }
  return;
