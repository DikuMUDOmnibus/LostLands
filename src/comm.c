...
void game_loop(int mother_desc)
{
  ...
  while (circle_shutdown != 1) {
	...
	case (3600 RL_SEC):
		send_to_all("\007\007LOSTLANDS WILL SHUTDOWN IN 1 HOUR\r\n");
		break;
	case (1800 RL_SEC):
		send_to_all("\007\007LOSTLANDS WILL SHUTDOWN IN 30 MINUTES\r\n");
		break;
	case (1200 RL_SEC):
		send_to_all("\007\007LOSTLANDS WILL SHUTDOWN IN 20 MINUTES\r\n");
		send_to_all("We suggest you go and RENT out soon.  We won't reimburse crash-lost items.\r\n");
		break;
	case (600 RL_SEC):
		send_to_all("\007\007LOSTLANDS WILL SHUTDOWN IN 10 MINUTES\r\n");
		send_to_all("We suggest you go and RENT out soon.  We won't reimburse crash-lost items.\r\n");
		break;
	case (300 RL_SEC):
		send_to_all("\007\007LOSTLANDS WILL SHUTDOWN IN 5 MINUTES\r\n");
		send_to_all("We suggest you go and RENT out soon.  We won't reimburse crash-lost items.\r\n");
		break;
	case (180 RL_SEC):
		send_to_all("\007\007LOSTLANDS WILL SHUTDOWN IN 3 MINUTES\r\n");
		send_to_all("We suggest you go and RENT out soon.  We won't reimburse crash-lost items.\r\n");
		break;
	case (60 RL_SEC):
		send_to_all("\007\007LOSTLANDS WILL SHUTDOWN IN 1 MINUTE\r\n");
		send_to_all("We suggest you go and RENT out soon.  We won't reimburse crash-lost items.\r\n");
		break;
	case (30 RL_SEC):
		send_to_all("\007\007LOSTLANDS WILL SHUTDOWN IN 30 SECONDS\r\n");
		break;
	case (10 RL_SEC):
		send_to_all("\007\007LOSTLANDS WILL SHUTDOWN IN 10 SECONDS\r\n");
		break;
	case (9 RL_SEC):
		send_to_all("\007\007LOSTLANDS WILL SHUTDOWN IN 9 SECONDS\r\n");
		break;
	case (8 RL_SEC):
		send_to_all("\007\007LOSTLANDS WILL SHUTDOWN IN 8 SECONDS\r\n");
		break;
	case (7 RL_SEC):
		send_to_all("\007\007LOSTLANDS WILL SHUTDOWN IN 7 SECONDS\r\n");
		break;
	case (6 RL_SEC):
		send_to_all("\007\007LOSTLANDS WILL SHUTDOWN IN 6 SECONDS\r\n");
		break;
	case (5 RL_SEC):
		send_to_all("\007\007LOSTLANDS WILL SHUTDOWN IN 5 SECONDS\r\n");
		break;
	case (4 RL_SEC):
		send_to_all("\007\007LOSTLANDS WILL SHUTDOWN IN 4 SECONDS\r\n");
		break;
	case (3 RL_SEC):
		send_to_all("\007\007LOSTLANDS WILL SHUTDOWN IN 3 SECONDS\r\n");
		break;
	case (2 RL_SEC):
		send_to_all("\007\007LOSTLANDS WILL SHUTDOWN IN 2 SECONDS\r\n");
		break;
	case (1 RL_SEC):
		send_to_all("\007\007SHUTTING DOWN\007\007\r\n");
		send_to_all("Come back soon, ya hear?\r\n");
		break;
	}
	circle_shutdown--;
    }
    ...
    if (!(pulse % (SECS_PER_MUD_HOUR * PASSES_PER_SEC / 3)))
      point_update();
...
void make_prompt(struct descriptor_data * d)
{
  ...
  if (d->str)
    ...
    *prompt = '\0';
    ...
    if (PRF2_FLAGGED(d->character, PRF2_AFK))
      sprintf(prompt, "%sAFK ", prompt);

    i = 100 * GET_HIT(d->character) / GET_MAX_HIT(d->character);
    STATUS_COLOR(i, colorbuf, d->character, C_SPR);
    if (PRF_FLAGGED(d->character, PRF_DISPHP))
      sprintf(prompt, "%s%s%d%s/%dh ", prompt, colorbuf,
	      GET_HIT(d->character),
	      CCNRM(d->character, C_SPR),
	      GET_MAX_HIT(d->character));

    i = 100 * GET_MANA(d->character) / GET_MAX_MANA(d->character);
    STATUS_COLOR(i, colorbuf, d->character, C_SPR);
    if (PRF_FLAGGED(d->character, PRF_DISPMANA))
      sprintf(prompt, "%s%s%d%sm ", prompt, colorbuf,
	      GET_MANA(d->character),
	      CCNRM(d->character, C_SPR));

    i = 100 * GET_MOVE(d->character) / GET_MAX_MOVE(d->character);
    STATUS_COLOR(i, colorbuf, d->character, C_SPR);
    if (PRF_FLAGGED(d->character, PRF_DISPMOVE))
      sprintf(prompt, "%s%s%d%sv ", prompt, colorbuf,
	      GET_MOVE(d->character),
	      CCNRM(d->character, C_SPR));

    if (PRF_FLAGGED(d->character, PRF_DISPGOLD))
      sprintf(prompt, "%s%s$%d%s ", prompt, 
	      CCYEL(d->character, C_SPR), 
	      GET_GOLD(d->character),
	      CCNRM(d->character, C_SPR));

    if (PRF_FLAGGED(d->character, PRF_DISPXP))
	if (GET_LEVEL(d->character) < (LVL_IMMORT - 1) && !IS_MOB(d->character))
	      sprintf(prompt, "%sXPtoLv %d: %d ", prompt, 
		 GET_LEVEL(d->character) + 1,
		 (titles[(int) GET_CLASS_NUM_FULL(d->character)][GET_LEVEL(d->character) + 1].exp) - GET_EXP(d->character) );

    if (PRF_FLAGGED(d->character, PRF_DISPDAM))
      if (!FIGHTING(d->character))
	sprintf(prompt, "%s[none] ", prompt);
      else {
	i = 100 * GET_HIT(FIGHTING(d->character)) / GET_MAX_HIT(FIGHTING(d->character));
	STATUS_COLOR(i, colorbuf, d->character, C_SPR);
	sprintf(prompt, "%s%s%d0%s%% ", prompt, colorbuf, i / 10, CCNRM(d->character, C_SPR));
      }
...
void close_socket(struct descriptor_data * d)
{
  ...
  if (d->character) {
    ...
    if (d->connected == CON_PLAYING || d->connected == CON_IEDIT
...
void signal_setup(void)
{
  ...
  my_signal(SIGALRM, shuttingdown);
}
