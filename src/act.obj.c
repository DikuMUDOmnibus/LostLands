...
ACMD(do_drink)
{
  ...
  if ((GET_COND(ch, DRUNK) > 30) && (GET_COND(ch, THIRST) > 0)) {
    /* The pig is drunk */
    send_to_char("You can't seem to get close enough to your mouth.\r\n", ch);
    act("$n tries to drink but misses $s mouth!", TRUE, ch, 0, 0, TO_ROOM);
    return;
  }
  if ((GET_COND(ch, FULL) > 60) && (GET_COND(ch, THIRST) > 0)) {
  ...
  gain_condition(ch, DRUNK,
	 (int) drink_aff[GET_OBJ_VAL(temp, 2)][DRUNK] * amount * 3);

  gain_condition(ch, FULL,
	 (int) drink_aff[GET_OBJ_VAL(temp, 2)][FULL] * amount * 3);

  gain_condition(ch, THIRST,
	 (int) drink_aff[GET_OBJ_VAL(temp, 2)][THIRST] * amount * 3);

  if (GET_COND(ch, DRUNK) > 30)
    send_to_char("You feel drunk.\r\n", ch);

  if (GET_COND(ch, THIRST) > 60)
    send_to_char("You don't feel thirsty any more.\r\n", ch);

  if (GET_COND(ch, FULL) > 60)
...
ACMD(do_eat)
{
  ...
  if (GET_COND(ch, FULL) > 60) {/* Stomach full */
  ...
  gain_condition(ch, FULL, amount * 3);
...
void wear_message(struct char_data * ch, struct obj_data * obj, int where)
{
  char *wear_messages[][2] = {
    ...
    {"$n wears $p in $s arm as a shield.",
    ...
    {"$n hoists $p onto $s back.",
     "You hoist $p onto your back."},
...
void perform_wear(struct char_data * ch, struct obj_data * obj, int where)
{
  ...
  int wear_bitvectors[] = {
    ...
    ITEM_WEAR_WIELD, ITEM_WEAR_WIELD, ITEM_WEAR_TAKE, ITEM_WEAR_BACK,
    ITEM_WEAR_FACE};
  ...
  char *already_wearing[] = {
    ...
    "You're already using something with your shield hand.\r\n",
    "You're already wearing something about your body.\r\n",
    "You already have something around your waist.\r\n",
    "YOU SHOULD NEVER SEE THIS MESSAGE.  PLEASE REPORT.\r\n",
    "You're already wearing something around both of your wrists.\r\n",
    "You're already wielding a weapon.\r\n",
    "You're already wielding a weapon in your off-hand.\r\n",
    "You're already holding something.\r\n",
    "You're already wearing something on your back.\r\n",
    "You already have something on your face.\r\n"
  };
  ...
  if (where == WEAR_DUALWIELD) {
	if (IS_OBJ_STAT2(obj, ITEM2_2HANDED)) {
		act("You cannot dual wield $p because it's a two-handed weapon.", FALSE, ch, obj, 0, TO_CHAR);
		return;
        } else if (GET_SKILL(ch, SKILL_DUAL_WIELD) <= 50) {
		act("You cannot dual wield $p yet.", FALSE, ch, obj, 0, TO_CHAR);
		return;
        } else if ((obj2 = GET_EQ(ch, WEAR_SHIELD))) {
		act("You're already using $p.", FALSE, ch, obj2, 0, TO_CHAR);
		return;
	}
  }
  ...
  if (where == WEAR_SHIELD && (obj2 = GET_EQ(ch, WEAR_WIELD)) && IS_OBJ_STAT2(obj2, ITEM2_2HANDED)) {
    act("You are already using $p, which is a two handed weapon!", FALSE, ch, obj2, 0, TO_CHAR);
    return;
  }
  if (where == WEAR_SHIELD && (obj2 = GET_EQ(ch, WEAR_DUALWIELD))) {
    act("You're already using $p... remove it first.", FALSE, ch, obj2, 0, TO_CHAR);
    return;
  }
  if (IS_OBJ_STAT2(obj, ITEM2_ENGRAVED) && (strcmp(obj->owner_name, GET_NAME(ch)) != 0)) {
    send_to_char("But that's not yours, it's engraved to someone else!\r\n", ch);
    return;
  }
...
int find_eq_pos(struct char_data * ch, struct obj_data * obj, char *arg)
{
  ...
  if (!arg || !*arg) {
    ...
    if (CAN_WEAR(obj, ITEM_WEAR_BACK))	      where = WEAR_BACKPACK;
...
ACMD(do_wield)
{
  ...
  if (!*arg)
    ...
    if (IS_MONK(ch)) 
      send_to_char("You can't wield that, you're a monk.\r\n", ch);
    ...
    else if (IS_OBJ_STAT2(obj, ITEM2_2HANDED) && GET_EQ(ch, WEAR_SHIELD))
      send_to_char("Your other hand is already full.", ch);
    else
