...
void	purge(char *filename, char *playername)
{
   FILE * fl;
   FILE * outfile;
...
{
   printf("NEW PURGE PLAY");
   if (argc > 3 || argc < 1)
      printf("Usage: %s playerfile-name player-name\n", argv[0]);
   else if (argc == 1)
	purge("players", "NO-ONE");
   else if (argc == 2)
        purge(argv[1], "NO-ONE");
   else
	purge(argv[1], argv[2]);
